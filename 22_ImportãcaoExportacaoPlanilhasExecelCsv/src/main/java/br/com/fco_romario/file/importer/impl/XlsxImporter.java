...
public class XlsxImporter implements FileImporter {
    ...
    public List<PersonDTO> importFile(InputStream inputStream) throws Exception {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook(inputStream)) {
            XSSFSheet sheet = workbook.getSheetAt(0);// neste caso estou considerando a o xlsx so tem uma aba.caso haja mais, considerar mais de uma abas

            Iterator<Row> rowIterator = sheet.iterator();

            if(rowIterator.hasNext()) rowIterator.next(); // pula o cabeçalho e pega apenas as linhas
    ...
    private List<PersonDTO> parseRowsToPersonDTOList(Iterator<Row> rowIterator) {
        ...
        while (rowIterator.hasNext()) {
            ...
            if(isRowValid(row)) {
                people.add(parseRowToPersoDTO(row));
            }
        }
    ...
    private PersonDTO parseRowToPersoDTO(Row row) {
